 ...
 */
public class ClusterEvaluationEX 
  ...
  public Instances DeleteNoise(Instances data){
	  noise = data.stringFreeStructure();
  ...
  /**
   * Return the instances with ID
   * 
   */
  public Instances getID(Instances data,int num){
	  ID = new CommonMethords().addIDInfo(data,new CommonMethords().InstanceToArrays(data));
	  for(int i=0;i<m_clusterAssignments.length;i++){
		  if(m_clusterAssignments[i]!=-1){
			  ID.instance(i).setValue(0, m_clusterAssignments[i]+num);}
		  else
			  ID.instance(i).setValue(0, m_clusterAssignments[i]);
	  }
	  return ID;
  }
  ...
  public static void saveClusterer(String fileName, 
                             Clusterer clusterer, 
  ...
  public boolean equals(Object obj) {
    ...
    ClusterEvaluationEX cmp = (ClusterEvaluationEX) obj;
